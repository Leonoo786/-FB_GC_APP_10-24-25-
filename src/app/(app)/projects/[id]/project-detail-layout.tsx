...
export default function ProjectDetailLayoutClient({
  projectId,
  children,
}: {
  projectId: string;
  ...
  const totalBudget = projectBudgetItems.reduce(
    (acc, item) => acc + item.originalBudget + item.approvedCOBudget,
    0
  );
  const spentToDate = projectExpenses.reduce(
  ...
  const budgetProgress =
    totalBudget > 0
      ? Math.min(Math.max((spentToDate / totalBudget) * 100, 0), 100)
      : 0;
  ...
  const timeProgress =
    totalDays > 0 ? Math.min(Math.max((daysPassed / totalDays) * 100, 0), 100) : 0;
  ...
  const budgetChartData = [
    ...
    {
      name: 'Remaining',
      value: Math.max(0, totalBudget - spentToDate),
      fill: 'hsl(var(--muted))',
    },
  ];
  ...
  const profitChartData = [
    ...
    {
      name: 'Profit',
      value: Math.max(0, profitAndLoss),
        ...
        <div className="flex justify-between items-start">
          ...
          <div className="flex gap-4 items-center">
            <ProjectSummaryChart
              ...
              metricLabel={`${budgetProgress.toFixed(
                1
        ...
        <CardContent className="pt-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6">
          <div className="lg:col-span-1">
            ...
            <p className="text-2xl font-bold">
              ${(project.finalBidAmount || 0).toLocaleString()}
            </p>
          ...
          <div className="space-y-4 lg:col-span-1">
            <div>
              ...
              <div className="flex justify-between text-xs text-muted-foreground mt-1">
                <span>
                  {daysPassed > 0 ? `${daysPassed} days passed` : 'Starting soon'}
                </span>
                <span>
                  {daysRemaining > 0
            ...
            <div>
              ...
              <div className="flex justify-between text-xs text-muted-foreground mt-1">
                ...
                <span>
                  ${(totalBudget - spentToDate).toLocaleString()} left
                </span>
              </div>
            </div>
          </div>
